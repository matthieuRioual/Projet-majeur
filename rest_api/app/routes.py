...
def get_incendies_par_id(incendie_id):
    if request.method == 'GET':
        incendies = Incendies.query.filter_by(id=incendie_id)
        resultat = dict()
        for i in incendies :
            dictionnaire_intermediaire = dict()
            dictionnaire_intermediaire["position_x"] = i.position_x
...
@app.route('/rest_api/v1.0/incendies/suppression/<int:incendie_id>', methods=['GET','DELETE','POST'])
