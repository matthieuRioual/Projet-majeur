...
@app.route('/rest_api/v1.0/vehicules', methods=['GET','POST']) # Permet d'obtenir la liste de tout les véhicules
def get_vehicules():
...
@app.route('/rest_api/v1.0/personnel/infos', methods=['GET', 'DELETE','PUT']) # renvoie la liste de tout les personnels
def get_personnels():
    if request.method == 'GET':
        personnels = Personnel.query.all()
        resultat = []
...
def creer_sonde():
    if request.method == 'POST':
        sonde = Sonde(position_x=request.json['position_x'], position_y=request.json['position_y'], etat=request.json['etat'], alarme=0)
